...
function SkeletonCard() {
  return (
    <div className="bg-surface rounded-[2rem] p-8 border border-border space-y-4">
      <div className="h-5 w-1/3 bg-surface-container animate-pulse rounded" />
  ...
  return (
    <InvestorLayout>
          ...
          <p className="font-['Inter'] text-foreground-variant text-lg leading-relaxed">
            ...
            {returnRate > 0 && (
              <> Your portfolio is performing{" "}
                <span className="text-accent font-bold">+{returnRate.toFixed(1)}%</span> return.
              </>
            )}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-12">
        ...
        <div className="md:col-span-4 bg-surface-container p-8 rounded-[2rem] flex flex-col justify-between relative overflow-hidden group border border-border">
          <div className="absolute -right-8 -top-8 w-32 h-32 bg-accent/10 rounded-full blur-3xl group-hover:bg-accent/20 transition-colors" />
          <div>
            <span className="font-['Inter'] text-xs font-bold tracking-widest text-accent uppercase mb-1 block">
              Total Return Rate
            </span>
            {dashLoading ? (
              <div className="h-16 w-32 bg-white/5 animate-pulse rounded mt-2" />
            ) : (
              <>
                <h3 className="font-['Hanken_Grotesk'] text-6xl font-black text-foreground mb-2">
                  {returnRate.toFixed(1)}<span className="text-accent">%</span>
                </h3>
          ...
          <div className="mt-8">
            ...
            <div className="flex justify-between mt-2 text-[10px] font-bold font-['Inter'] text-foreground-variant uppercase tracking-tighter">
              <span>Target 15%</span>
        ...
        <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-surface p-8 rounded-[2rem] border border-border">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="font-['Hanken_Grotesk'] font-bold text-foreground">Capital Contribution</span>
            </div>
            {dashLoading ? (
              <div className="h-10 w-40 bg-surface-container animate-pulse rounded" />
            ) : (
              <>
                <div className="flex items-baseline gap-2">
                  <span className="font-['Hanken_Grotesk'] text-4xl font-black text-foreground">
                    ${totalCapital.toLocaleString("en-SG", { minimumFractionDigits: 0 })}
                  </span>
                  <span className="font-['Inter'] text-sm text-foreground-variant">SGD</span>
                </div>
                <p className="mt-4 font-['Inter'] text-sm text-foreground-variant">
          ...
          <div className="bg-surface p-8 rounded-[2rem] border border-border">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="font-['Hanken_Grotesk'] font-bold text-foreground">Total Distributions</span>
            </div>
            {dashLoading ? (
              <div className="h-10 w-40 bg-surface-container animate-pulse rounded" />
            ) : (
      ...
      {investments.length > 0 && (
        <section className="mb-16">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-['Hanken_Grotesk'] text-2xl font-bold text-foreground">
              Portfolio Assets
            </h3>
          </div>
      ...
      <section className="bg-surface rounded-[2rem] p-10 border border-border">
