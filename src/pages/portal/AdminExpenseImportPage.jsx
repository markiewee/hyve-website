...
export default function AdminExpenseImportPage() {
  ...
  return (
    <PortalLayout>
      {/* Page header */}
      <div className="mb-8">
        <p className="font-['Inter'] text-[10px] uppercase tracking-widest text-foreground-variant font-bold mb-1">
          Accounting
        </p>
      ...
      {/* Finalized banner */}
      {isFinalized && (
        <div className="mb-6 px-5 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/25 flex items-center gap-2">
          <span className="material-symbols-outlined text-emerald-300 text-[18px]">verified</span>
          <span className="font-['Inter'] text-sm font-semibold text-emerald-300">
      ...
      <div className="bg-surface rounded-2xl border border-border overflow-hidden">
        <div className="px-8 py-6 border-b border-border flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          {activeTab === TAB_ASPIRE && (
            <div className="flex items-center gap-3 flex-wrap">
              {accountsLoading ? (
                <div className="h-10 bg-surface-container rounded-xl animate-pulse w-48" />
              ) : accountsError ? (
                <span className="font-['Inter'] text-xs text-red-400">{accountsError}</span>
              ) : (
                <select
                  value={selectedAccount}
                  onChange={(e) => setSelectedAccount(e.target.value)}
                  className="px-3 py-2 rounded-lg border border-border text-sm font-['Inter'] text-foreground focus:outline-none focus:ring-2 focus:ring-accent bg-surface-container max-w-[220px]"
                >
              ...
              <input
                ...
                className="px-3 py-2 rounded-lg border border-border text-sm font-['Inter'] text-foreground bg-surface-container focus:outline-none focus:ring-2 focus:ring-accent"
              />
              <button
              ...
              {hasFetched && (
                <button
                  ...
                  disabled={savingAll}
                  className="px-5 py-2.5 bg-amber-500/15 text-amber-300 border border-amber-500/25 rounded-xl font-['Inter'] font-bold text-sm hover:bg-amber-500/25 disabled:opacity-50 transition-all flex items-center gap-2 shrink-0"
                >
              ...
              {hasFetched && (
                <button
                  ...
                  className="px-3 py-2.5 text-foreground-variant hover:text-red-300 rounded-xl font-['Inter'] font-bold text-xs transition-colors flex items-center gap-1 shrink-0"
        ...
        {/* Error */}
        {fetchError && (
          <div className="px-8 py-3 bg-red-500/10 text-red-300 font-['Inter'] text-sm">
            {fetchError}
          </div>
        )}
        ...
        {activeTab === TAB_CSV && (
          <div className="p-8">
            {csvError && (
              <p className="font-['Inter'] text-sm text-red-400 mb-4">{csvError}</p>
            )}
            <CsvUploader
          ...
          <>
            ...
            {fetching && (
              <div className="px-8 py-8 flex items-center justify-center gap-3">
                ...
                <span className="font-['Inter'] text-sm text-foreground-variant">
                  ...
                  <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
                    {untagged.length === 0 ? (
                      ...
                      untagged.map((txn) => {
                        ...
                        return (
                          <div
                            key={txn._key}
                            className="rounded-xl border p-4 transition-all bg-surface-container border-border"
                          >
                            {/* Transaction header */}
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1 min-w-0">
                                <p className="font-['Inter'] font-bold text-sm text-foreground truncate">
                                  {txn.description || "Unknown"}
                                </p>
                                <p className="font-['Inter'] text-xs text-foreground-variant mt-0.5">
                                  {formatDate(txn.transaction_date)}
                                  {txn.reference && (
                                    <span className="ml-2 text-foreground-variant">
                                      Ref: {txn.reference}
                                    </span>
                                  )}
                                </p>
                              </div>
                              <p className="font-['Hanken_Grotesk'] font-bold text-sm shrink-0 text-red-400 tabular-nums">
                              ...
                              <div className="mt-3 space-y-2">
                                <div className="flex flex-wrap gap-2">
                                  ...
                                  <select
                                    ...
                                    className="flex-1 min-w-[120px] px-2 py-1.5 rounded-lg border border-border text-xs font-['Inter'] text-foreground focus:outline-none focus:ring-2 focus:ring-accent bg-surface"
                                  >
                                  ...
                                  {edit.property_id && (
                                    <select
                                      ...
                                      className="min-w-[100px] px-2 py-1.5 rounded-lg border border-border text-xs font-['Inter'] text-foreground focus:outline-none focus:ring-2 focus:ring-accent bg-surface"
                                    >
                                  ...
                                  <select
                                    ...
                                    className="min-w-[110px] px-2 py-1.5 rounded-lg border border-border text-xs font-['Inter'] text-foreground focus:outline-none focus:ring-2 focus:ring-accent bg-surface"
                                  >
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    className="px-3 py-1.5 text-foreground-variant text-xs font-bold rounded-lg hover:bg-white/5"
                                  >
                                    Ignore
                                  </button>
                                </div>
                    ...
                    {ignored.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-border">
                        <p className="font-['Inter'] text-[10px] uppercase tracking-widest text-foreground-variant font-bold mb-2">
                          Ignored ({ignored.length})
                        </p>
                        {ignored.map((txn) => (
                          <div
                            key={txn._key}
                            className="flex items-center justify-between gap-2 py-2 px-3 rounded-lg bg-surface-container mb-1"
                          >
                            <div className="flex-1 min-w-0">
                              <p className="font-['Inter'] text-xs text-foreground-variant truncate">
                                {txn.description}
                              </p>
                            </div>
                            <span className="font-['Hanken_Grotesk'] text-xs text-foreground-variant tabular-nums shrink-0">
                              {formatSGD(txn.amount)}
                            </span>
                            <button
                              onClick={() => handleUnignore(txn)}
                              className="text-[10px] text-accent font-bold hover:underline shrink-0"
                            >
                              Undo
                            </button>
                          </div>
                    ...
                    {tagged.length === 0 ? (
                      ...
                      Object.entries(
                        ...
                        <div key={propId} className="mb-4">
                          <p className="font-['Inter'] text-[10px] uppercase tracking-widest text-accent font-bold mb-2 flex items-center gap-2">
                          ...
                          <div className="space-y-1">
                            {txns.map((txn) => (
                              <div
                                key={txn._key}
                                className="flex items-center gap-3 py-2.5 px-3 rounded-xl bg-emerald-500/10 border border-emerald-500/25"
                              >
                                ...
                                {txn._accrualMonth && txn._accrualMonth !== reconcileMonth && (
                                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest bg-amber-500/15 text-amber-300" title="Accounting period differs from reconciliation month">
                                    {txn._accrualMonth}
                                  </span>
                                )}
                                <p className="font-['Inter'] text-xs text-foreground flex-1 min-w-0 truncate">
                                  {txn.description}
                                </p>
                                <p className="font-['Hanken_Grotesk'] font-bold text-xs tabular-nums text-foreground shrink-0">
                                  {formatSGD(txn.amount)}
                                </p>
                                <button
      ...
      {hasFetched && Object.keys(summary).length > 0 && (
        <div className="bg-surface rounded-2xl border border-border overflow-hidden mt-8">
          <div className="px-8 py-6 border-b border-border flex items-center justify-between">
            <div>
              <h2 className="font-['Hanken_Grotesk'] font-bold text-lg text-foreground">
                Summary by Property
              </h2>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <p className="font-['Hanken_Grotesk'] font-bold text-sm text-foreground">
                        {getPropertyName(propId)}
                      </p>
                      <p className="font-['Hanken_Grotesk'] font-bold text-sm text-accent tabular-nums">
                        {formatSGD(propTotal)}
                      </p>
                    </div>
                        ...
                        <div key={month} className={sortedMonths.length > 1 ? "mb-3 last:mb-0" : ""}>
                          {sortedMonths.length > 1 && (
                            <div className="flex items-center justify-between mb-1">
                              <span className="font-['Inter'] text-[9px] uppercase tracking-widest text-accent font-bold">
                                {formatMonthLabel(month)}
                              </span>
                              <span className="font-['Hanken_Grotesk'] text-[10px] font-semibold text-accent tabular-nums">
                          ...
                          <div className="space-y-1">
                            {Object.entries(cats)
                              ...
                              .map(([cat, amt]) => (
                                ...
                                >
                                  <span className="font-['Inter'] text-foreground-variant">
                                    {categoryLabel(cat)}
                                  </span>
                                  <span className="font-['Hanken_Grotesk'] font-semibold text-foreground tabular-nums">
                                    {formatSGD(amt)}
                                  </span>
                                </div>
                              ))}
      ...
      {showPnl && pnlData && (
        <div className="bg-surface rounded-2xl border border-border overflow-hidden mt-8">
          <div className="px-8 py-6 border-b border-border flex items-center justify-between">
            ...
            {isFinalized ? (
              <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-500/15 text-emerald-300 font-['Inter'] font-bold text-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-3 pr-4 font-['Inter'] text-[10px] uppercase tracking-widest text-foreground-variant font-bold">
                    &nbsp;
                  </th>
                  {pnlData.properties.map((propId) => (
                    <th
                      key={propId}
                      className="text-right py-3 px-4 font-['Inter'] text-[10px] uppercase tracking-widest text-foreground-variant font-bold"
                    >
                      {getPropertyName(propId)}
                    </th>
                  ))}
                  <th className="text-right py-3 pl-4 font-['Inter'] text-[10px] uppercase tracking-widest text-accent font-bold">
                    Total
                  </th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr className="bg-accent/10">
                  <td
                    ...
                    className="py-2 px-0 font-['Inter'] text-[10px] uppercase tracking-widest text-accent font-bold"
                  >
                    Income
                  </td>
                </tr>
                <tr className="border-b border-border">
                  <td className="py-2 pr-4 font-['Inter'] text-foreground">
                    Rent Collected
                  </td>
                  {pnlData.properties.map((propId) => (
                    <td
                      key={propId}
                      className="py-2 px-4 text-right font-['Hanken_Grotesk'] font-semibold text-foreground tabular-nums"
                    >
                      {formatSGD(pnlData.income[propId] || 0)}
                    </td>
                  ))}
                  <td className="py-2 pl-4 text-right font-['Hanken_Grotesk'] font-bold text-accent tabular-nums">
                    {formatSGD(
                ...
                <tr className="bg-red-500/10">
                  <td
                    colSpan={pnlData.properties.length + 2}
                    className="py-2 px-0 font-['Inter'] text-[10px] uppercase tracking-widest text-red-400 font-bold"
                  >
                    Expenses
                  </td>
                </tr>
                {pnlData.categories.filter((cat) => cat !== "MANAGEMENT_FEES").map((cat) => (
                  <tr key={cat} className="border-b border-border">
                    ...
                    <td className="py-2 pl-4 text-right font-['Hanken_Grotesk'] font-semibold text-foreground tabular-nums">
                      {formatSGD(
                        pnlData.properties.reduce(
                ...
                <tr className="border-b border-border">
                  <td className="py-2 pr-4 font-['Inter'] font-bold text-foreground">
                    Total Expenses
                  </td>
                  {pnlData.properties.map((propId) => {
                    const propExpenses = pnlData.expenses[propId] || {};
                    ...
                    return (
                      <td
                        key={propId}
                        className="py-2 px-4 text-right font-['Hanken_Grotesk'] font-bold text-red-400 tabular-nums"
                      >
                        {formatSGD(total)}
                      </td>
                    );
                  })}
                  <td className="py-2 pl-4 text-right font-['Hanken_Grotesk'] font-bold text-red-400 tabular-nums">
                    {formatSGD(
                  ...
                  <tr className="border-b border-border bg-amber-500/10">
                    ...
                    {pnlData.properties.map((propId) => {
                      ...
                      return (
                        <td
                          key={propId}
                          className="py-2 px-4 text-right font-['Hanken_Grotesk'] font-semibold tabular-nums text-amber-300"
                        >
                          {carried > 0 ? `(${formatSGD(carried)})` : "--"}
                        </td>
                      );
                    })}
                    <td className="py-2 pl-4 text-right font-['Hanken_Grotesk'] font-semibold tabular-nums text-amber-300">
                      {formatSGD(
                        pnlData.properties.reduce(
                      ...
                      return (
                        <td
                          ...
                          className={`py-2 px-4 text-right font-['Hanken_Grotesk'] font-semibold tabular-nums ${
                            waived ? "text-amber-300 italic" : "text-foreground"
                          }`}
                        >
                ...
                {/* Net Profit */}
                <tr className="bg-accent/10">
                  <td className="py-3 pr-4 font-['Hanken_Grotesk'] font-bold text-lg text-foreground">
                    Net Profit
                  </td>
                  {pnlData.properties.map((propId) => {
                      ...
                      return (
                        <span
                          className={`font-['Hanken_Grotesk'] font-bold text-lg tabular-nums ${
                            totalNet >= 0 ? "text-accent" : "text-red-400"
                          }`}
                        >
                          {totalNet < 0
                ...
                {/* Notes per property */}
                <tr>
                  <td className="py-3 pr-4 align-top font-['Inter'] font-semibold text-foreground-variant text-xs pt-4">
                    Notes
                  </td>
                  {pnlData.properties.map((propId) => (
                    <td key={propId} className="py-3 px-4 align-top">
                      <textarea
                        ...
                        className="w-full px-3 py-2 rounded-lg border border-border text-xs font-['Inter'] text-foreground placeholder:text-foreground-variant focus:outline-none focus:ring-2 focus:ring-accent bg-surface resize-y disabled:bg-surface-container disabled:opacity-60"
                      />
                    </td>
                  ))}
