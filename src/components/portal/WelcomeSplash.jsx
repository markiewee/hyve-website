...
function SimpleMarkdown({ text }) {
  ...
  return (
    <div className="space-y-1.5">
      {lines.map((line, i) => {
        if (line.startsWith("## ")) return <h4 key={i} className="font-display font-bold text-foreground text-sm mt-2 mb-1">{line.slice(3)}</h4>;
        if (line.trim() === "") return <div key={i} className="h-1" />;
        const parts = line.split(/(\*\*[^*]+\*\*)/g);
        return (
          <p key={i} className="text-sm text-foreground-variant leading-relaxed">
            {parts.map((part, j) =>
    ...
    <div className="space-y-6">
      <div className="text-center py-4">
        <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-accent/10 flex items-center justify-center">
          <span
            className="material-symbols-outlined text-accent text-[36px]"
            style={{ fontVariationSettings: "'FILL' 1" }}
          >
            celebration
          </span>
        ...
        <p className="text-sm text-foreground-variant max-w-md mx-auto leading-relaxed">
