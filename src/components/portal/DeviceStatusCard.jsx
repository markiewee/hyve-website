...
export default function DeviceStatusCard({ device, energyReading }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-[#DDD0AD] shadow-sm space-y-3">
      {/* Header row */}
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-['Inter'] font-bold text-[#181511]">{roomName}</span>
          </div>
          {propertyName && (
            <p className="text-xs text-[#57534E] font-['Inter'] mt-0.5">{propertyName}</p>
          )}
        </div>
      ...
      {hasEnergy && (
        <div className="grid grid-cols-4 gap-2 bg-[#FAF6EC] rounded-xl p-3">
          <div className="text-center">
            <p className="font-['Hanken_Grotesk'] text-lg font-bold text-[#181511]">{Number(current).toFixed(1)}</p>
            <p className="text-[10px] text-[#57534E] font-['Inter'] uppercase tracking-wider">Amps</p>
          </div>
          <div className="text-center">
            <p className="font-['Hanken_Grotesk'] text-lg font-bold text-[#181511]">{Math.round(Number(power || 0))}</p>
            <p className="text-[10px] text-[#57534E] font-['Inter'] uppercase tracking-wider">Watts</p>
          </div>
          <div className="text-center">
            <p className="font-['Hanken_Grotesk'] text-lg font-bold text-[#181511]">{Math.round(Number(voltage || 0))}</p>
            <p className="text-[10px] text-[#57534E] font-['Inter'] uppercase tracking-wider">Volts</p>
          </div>
          <div className="text-center">
            <p className="font-['Hanken_Grotesk'] text-lg font-bold text-[#A87813]">{Number(energy || 0).toFixed(1)}</p>
            <p className="text-[10px] text-[#57534E] font-['Inter'] uppercase tracking-wider">kWh</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs">
        ...
        {/* Last seen */}
        <span className="text-[#57534E] font-['Inter'] ml-auto">
