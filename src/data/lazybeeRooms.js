...
/** Hero photograph per home, used by the carousel and the homes strip. */
export const HOME_HERO = {
  CP: "/photos/cp-hero.jpg",
  IH: "/photos/ih-hero.jpg",
  TG: "/photos/tg-hero.jpg",
};

export const roomsForHome = (code) => ROOMS.filter((r) => r.home === code);

/**
 * A room is let when its next available date is still in the future.
 * The prototype froze "today" at the pull date; on a live page the real date is
 * the honest one, so it defaults to now and stays injectable for tests.
 */
export const isLet = (room, today = new Date()) => new Date(room.next) > today;
